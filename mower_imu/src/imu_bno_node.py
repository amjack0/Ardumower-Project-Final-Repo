...
def read_imu_raw_data():
	global accl_x, accl_y,accl_z, gyro_x, gyro_y, gyro_z, comp_x, comp_y, comp_z, quad_x, quad_y, quad_z, quad_w, quad_norm;
	...
	gyro_x = gyro[0] * pi / 180
	gyro_y = gyro[1] * pi / 180
	gyro_z = gyro[2] * pi / 180
	...
	quaternion = bno.read_quaternion()
    	quad_norm = pow(
        	...
        	pow(quaternion[1], 2) +
        	pow(quaternion[2], 2) +
        	pow(quaternion[3], 2), 0.5)
	quad_norm = quad_norm if quad_norm else 1
	quad_x = quaternion[0] / quad_norm
	quad_y = quaternion[1] / quad_norm
	quad_z = quaternion[2] / quad_norm
	quad_w = quaternion[3] / quad_norm
